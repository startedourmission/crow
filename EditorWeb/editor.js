...
const filenameTitle=fileTitle(title=>new Promise((resolve,reject)=>{
  ...
  let replacement;
  try{if(crowmapNote)replacement=renamedNoteSource(source,title);}
  catch(e){reject(e);return;}
  renamePending={resolve,reject};send({action:'rename',title,expected:source,...(replacement!=null?{source:replacement}:{})});
}));
filenameTitle.set('');main.prepend(filenameTitle.dom);
...
function receive(value, blocks, fontSize, linksEnabled = false) {
  ...
  try {
    ...
    for (const block of blocks) {
      ...
      parsed = wikiNodes(parsed, noteLinksEnabled);
