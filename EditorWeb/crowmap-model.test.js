...
test('Start-to-first milestone edges stay straight while later segments still bend',()=>{
 ...
 assert.equal(layout.edgePoints.get(stem.id).length,2);assert.equal(layout.edgePoints.get(stem.id)[0],layout.points.get(start));assert.equal(layout.edgePoints.get(stem.id)[1],layout.points.get(a));
 assert(layout.edgePoints.get(later.id).length>=3);assert.equal(layout.edgePoints.get(later.id)[0],layout.points.get(a));assert.equal(layout.edgePoints.get(later.id).at(-1),layout.points.get(b));
 assert.notEqual(layout.points.get(start).y,layout.points.get(a).y);
