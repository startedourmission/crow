...
export function graphGestures({canvas,viewport,notes,selection,doc,layout,onSelect,onConnect,onMove,onReorder,preview,pause,resume}){
 ...
 canvas.addEventListener('pointerdown',event=>{
  ...
  overlay.style.display='none';canvas.append(overlay);if(node&&!port){canvas.prepend(lane);canvas.append(label);}let moved=false,target=null,reorder=null,days=0,priority,cascadePriority=event.metaKey;const main=mainMilestoneIDs(doc),originalPoints=new Map([...layout.points].map(([id,p])=>[id,{...p}]));
  ...
  pause();
  const move=e=>{
   if(Math.hypot(e.clientX-event.clientX,e.clientY-event.clientY)<4&&!moved)return;moved=true;cascadePriority=e.metaKey;overlay.style.display='';const p=point(e);
   if(port){
    ...
    reorder?.group.classList.remove('order-target');reorder=null;
    if(node.kind&&node.kind!=='start'&&!days){const original=originalPoints.get(node.id),targetX=original.x+p.x-start.x,targetY=original.y+p.y-start.y;
     const candidate=doc.anchors.filter(a=>a.id!==node.id&&a.kind!=='start'&&a.project===node.project&&a.date===node.date&&main.has(a.id)===main.has(node.id)).find(a=>{const q=originalPoints.get(a.id);return Math.abs(q.x-targetX)<36&&Math.abs(q.y-targetY)<24;});
    ...
    const z=canvas.getBoundingClientRect().width/canvas.viewBox.baseVal.width;attrs(label,{x:x+8,y:viewport.scrollTop/z+50});label.textContent=date+(reorder?' · Swap order':priority?' · Priority '+priority+(cascadePriority?' · Following milestones':''):'');lane.style.display=priority?'':'none';
    if(priority)attrs(lane,{x:0,y:layout.priorityY(priority)-28,width:canvas.viewBox.baseVal.width,height:56});
    dragged.forEach((n,i)=>{const dx=layout.x(new Date((day(positions[i].date)+days)*86400000).toISOString().slice(0,10))-layout.x(positions[i].date),dy=reorder?originalPoints.get(reorder.id).y-positions[i].y:priority!=null&&priority!==layout.rankAt(node.project,node.date)?layout.priorityY(priority)-positions[i].y:0;n.group.setAttribute('transform',`translate(${dx} ${dy}) ${transforms[i]}`);n.point.x=positions[i].x+dx;n.point.y=positions[i].y+dy;});preview();
   }else{
