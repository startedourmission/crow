...
function transaction(result,{links=true,draftNote=null}={}){if(links&&graphError)throw Error('Fix the note links before changing the plan. '+graphError);if(links)result=linkedTransaction(result,data.texts);if(pending)return Promise.reject(Error('Wait for the current change to save.'));validateMap(result.doc);return new Promise((resolve,reject)=>{pending={resolve,reject};send({action:'save',draftNote,source:JSON.stringify(result.doc,null,2)+'\n',expected:data.source,writes:result.writes??[],deletes:result.deletes??[],reads:result.reads??[],deletingProject:result.deletingProject??null});});}
async function sampleProject(){
 ...
 try {const result=createProject(data.doc,{title:'Sample project'+(data.doc.projects.length?' '+(data.doc.projects.length+1):''),date,priority:data.doc.projects.length+1,milestones:[
  ...
  {title:'Build',date:offset(21)},{title:'Release',date:offset(28)}]},Object.keys(data.texts));await transaction(result);selected=null;selectedNotes.clear();render();focusTimeline(result.doc.projects.at(-1).route[0]);}
...
async function newMilestone(fromID){try{if(!await flushNote())return;const result=addMilestone(data.doc,fromID,Object.keys(data.texts));await transaction(result);selected={kind:'anchor',id:result.doc.anchors.at(-1).id};render();}catch(e){notice(e.message);}}
async function addWork(attach){
 ...
 await transaction(result);selected={kind:'note',id};render();if(viewActive)embedded?.editor.commands.focus('end');
 }catch(e){notice(e.message);}
}
 ...
 try{
  ...
  const modal=dialog('Delete timeline?',(form,close)=>{
   ...
   form.onsubmit=async event=>{event.preventDefault();if(busy)return;busy=true;submit.disabled=true;cancel.disabled=true;
    try{if(data.source!==expected)throw Error('The timeline changed. Close this dialog and review it before deleting.');await transaction(deleteTimeline(data.doc,startID,data.texts),{links:false});selected=null;selectedNotes.clear();close();render();}
...
async function nodeMenu(event,item){
 ...
 action('Duplicate',async()=>{menu.remove();try{if(graphError)throw Error(graphError);const nodes=copyNodes(data.doc,ids);for(const n of nodes)if(data.drafts?.[n.note]&&!n.device)throw Error('Save the selected notes before duplicating.');const result=duplicateNodes(data.doc,ids,data.texts);await transaction(result,{links:false});selectedNotes.clear();for(const id of result.copiedIDs)if(data.doc.notes.some(n=>n.id===id))selectedNotes.add(id);selected=result.copiedIDs.length===1?{kind:item.kind?'anchor':'note',id:result.copiedIDs[0]}:null;render();}catch(e){notice(e.message);}});
 ...
 if(!item.kind){
  ...
  action(ids.length>1?'Delete '+ids.length+' notes':item.device?'Remove from map':'Delete note',async()=>{menu.remove();try{await transaction(deleteWorkNotes(data.doc,ids,data.texts),{links:false});selectedNotes.clear();render();}catch(e){notice(e.message);}});
 }
...
async function flushNote(){
 ...
 const source=state.read();state.saving=(async()=>{try{
  if(pending){state.status.textContent='Waiting to save…';state.timer=setTimeout(()=>flushNote(),100);return false;}
  let doc;try{doc=resolveMap(data.doc,{...data.texts,[state.name]:source});}catch{doc=data.doc;}
  await transaction({doc,writes:[{name:state.name,text:source,expected:data.texts[state.name]}]},{links:false,draftNote:state.name});
  state.dirty=state.read()!==source;state.status.textContent=state.dirty?'Saving…':'Saved';if(state.dirty)state.timer=setTimeout(()=>flushNote(),100);return true;
...
function render(){
 ...
 function timelineLine(points,cls,color){const element=svg('path',{d:curvePath(points),class:cls,fill:'none',stroke:color});timelineShapes.push({points,element});return element;}
 for(const edge of doc.edges){const points=layout.edgePoints.get(edge.id);
 const color=projects.get(edge.project).color,visible=timelineLine(points,'timeline-edge active',color);canvas.append(visible);const hit=timelineLine(points,'edge-hit','transparent');hit.dataset.edgeId=edge.id;interactive(hit,'Segment: '+layout.points.get(edge.from).title+' to '+layout.points.get(edge.to).title,()=>choose({kind:'edge',id:edge.id}));canvas.append(hit);
...
window.crowMap={flush:flushNote,setActive(active){viewActive=active;if(!active){stopMotion();document.activeElement?.blur();}else if(data)render();},addSampleProject:sampleProject,renamed(value){const pending=renamePending;renamePending=null;if(value.error){pending?.reject(Error(value.error));return;}if(embedded){clearTimeout(embedded.timer);embedded.destroy();embedded=null;}this.receive(value);pending?.resolve(value.name);},draftError(name,message){if(embedded?.name===name){embedded.blocked=true;embedded.status.textContent=message;}},async refreshDevice(value){
 ...
 if(JSON.stringify(doc)!==JSON.stringify(data.doc))await transaction({doc,writes:[]});
