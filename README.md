...
하단 오른쪽에서 Mac의 자동 절전을 방지하고 Crow 프로세스의 메모리·CPU를 확인할 수 있습니다.
...
`.canvas`와 `.base`도 기본으로 렌더링하며 상단 버튼으로 소스를 편집할 수 있습니다. Canvas는 텍스트·파일·링크·그룹 노드와 연결선, 확대·이동을 지원합니다. Bases는 현재 로컬/SSH 워크스페이스의 파일과 Markdown frontmatter로 table/cards/list 뷰, 필터·정렬·그룹·기본 수식을 표시합니다. 파일 변경은 미리보기 새로고침으로 반영합니다. Obsidian 플러그인 뷰·고급 수식 전체·요약 계산은 지원하지 않으며, 지원하지 않는 표현식은 오류로 표시합니다. 최대 2,000개 파일/3,000개 폴더, Markdown 파일당 512 KB·총 20 MB로 제한하고 생략한 파일은 안내합니다.
...
클라이언트와 서버 Mac 양쪽에 Crow가 필요합니다. 등록은 처음에 한 번 진행하며,
역방향 요청마다 승인 팝업이 나타나지는 않습니다.

1. **클라이언트 Mac**의 **Settings → Paired Server → Copy Public Key**를 누릅니다.
   공개키를 서버 Mac으로 전달합니다. 개인키는 클라이언트 Keychain을 벗어나지 않습니다.
2. **서버 Mac**의 **Settings → Crow Server**에 이 공개키를 붙여 넣습니다.
   양쪽 화면의 **Client fingerprint**가 같은지 확인하고 체크한 뒤 **Install Server**를 누릅니다.
   서명된 Crow 빌드와 서버 Mac의 관리자 인증이 필요합니다.
3. 서버 설정에서 사용할 에이전트의 **독립 실행형 네이티브 바이너리**를 등록합니다.
   npm 셸 래퍼·JavaScript 및 사용자 설치 라이브러리에 의존하는 실행 파일은 거절합니다.
   Codex npm 배포판은 내부 네이티브 `codex` 바이너리를 선택하세요. Grok도 같은 조건을 충족해야 합니다.
4. 서버의 **Pairing code → Copy**로 암호화된 등록 코드를 클라이언트에 전달합니다.
   클라이언트의 **Paired Server**에서 SSH 호스트와 코드를 입력하고, 양쪽의 **Server fingerprint**를
   비교해 일치할 때만 체크한 뒤 **Pair**를 누릅니다. 클립보드 내용을 바꿔치기하는 공격을 막기 위해
   두 단계의 지문 확인을 생략하지 마세요.
5. SSH 연결 후 워크스페이스의 **+ → Open Reverse Agent** 또는 호스트의 역방향 버튼을 누릅니다.
   에이전트와 허용할 **로컬 폴더**를 선택합니다. 로컬 명령 실행은 기본값이 꺼짐입니다.

서버 화면과 클립보드에는 해당 클라이언트 공개키로 암호화된 등록 코드만 나옵니다.
등록 코드를 다른 프로세스가 복사해도 클라이언트 개인키 없이는 복호화할 수 없습니다.
실제 연결 비밀값은 클라이언트 Keychain과 서버 root 전용 파일에만 저장하며,
에이전트 대화·명령행·일반 SSH 계정에 전달하지 않습니다. 클라이언트 변경은 새 공개키를
서버 설정에 등록하고 **Update Server**로 적용합니다. 이전 연결 키도 함께 폐기됩니다.

서버는 매 실행마다 로그인할 수 없는 별도 OS 계정을 만들고, 역방향 요청의 실제 사용자 ID를
커널에서 확인합니다. 다른 일반 SSH 로그인에서 `ssh` 명령과 비밀번호를 입력하는 것으로는
이 연결을 사용할 수 없습니다. 기존 공유 키·비밀번호 역방향 접속은 계속 차단되어 있습니다.
Linux·WSL·iOS는 지원하지 않습니다.

전용 에이전트 안에서는 다음 명령을 쓸 수 있습니다.

```sh
crow-reverse list
crow-reverse read notes.md
printf 'hello\n' | crow-reverse write result.txt
crow-reverse exec 'git status' # 시작할 때 로컬 명령 실행을 허용한 경우에만
```

파일 경로는 선택한 로컬 폴더 기준이며 심볼릭 링크·하드 링크는 거절합니다. 파일 및 명령 출력은
1 MB로 제한합니다. **명령 실행을 허용하면 로컬 사용자 계정 권한 전체로 실행되므로 폴더 제한을
벗어날 수 있습니다.** 실행할 에이전트와 서버 프로젝트를 신뢰할 때만 켜세요.

에이전트 종료·연결 단절 시 작업의 역방향 접근을 닫고 전용 사용자 프로세스와 프로젝트 ACL을
정리합니다. 앱 재시작 후에는 새 전용 에이전트를 명시적으로 열어야 합니다. 서버의 **Stop**으로
모든 작업을 종료하고, **Reset**으로 기존 페어링을 무효화할 수 있습니다.

각 실행은 별도 홈을 사용하므로 기존 SSH 계정의 에이전트 로그인 정보나 대화 기록을 가져오지
않습니다. CLI에서 로그인이 필요할 수 있으며, 종료 시 이 홈도 삭제합니다. 프로젝트는 서버의
`/Users` 아래 사용자 소유 폴더여야 합니다. 서버 관리자/root 및 에이전트가 실행하도록 유도한
프로젝트 코드, 클라이언트 Mac의 사용자 계정은 이 격리의 신뢰 경계 밖입니다. 동일 프로젝트에 대한 쓰기 권한을 가진 사용자의
코드 변경까지 안전하게 만드는 기능은 아닙니다.
...
- 기존 공유 키·비밀번호 방식의 Reverse SSH는 비활성화되었습니다.
  역방향 기능은 위의 Crow 서버 설치·페어링과 전용 에이전트 실행을 통해서만 사용할 수 있습니다.
