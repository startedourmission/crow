...
왼쪽 **Hosts → Agents → tmux → Server Screen** 순서로 배치되어 있습니다.
**Agents**는 로컬·SSH 폴더를 함께 보여주는 작업 공간 목록입니다. 작업 공간을 클릭하면
해당 폴더와 열린 파일·터미널로 전환하고, 아래 세션을 클릭하면 해당 터미널을 선택합니다.
작업 공간을 고정할 수 있으며 최근 사용 순서와 연결 상태도 표시합니다.

Agents의 **+**에서 로컬 폴더 또는 연결된 SSH 호스트의 폴더를 엽니다.
...
iPhone에서는 **Session and Settings → Agents / tmux**에서 엽니다.
...
**tmux** 패널은 현재 호스트의 기본 소켓을 사용하는 세션을 표시합니다. **+**로 생성하고,
