...
final class CrowAppTests: XCTestCase {
    ...
    @MainActor func testCreateNavigateRenameAndRecoverableDelete() async throws {
        ...
        let legacy = model.vaultURL.appendingPathComponent(".crow-trash")
        try FileManager.default.createDirectory(at: legacy, withIntermediateDirectories: false)
        try Data("keep old recovery".utf8).write(to: legacy.appendingPathComponent("old.txt"))
        await model.trash(renamed).value
        XCTAssertFalse(FileManager.default.fileExists(atPath: renamed.path))
        let trash = model.vaultURL.appendingPathComponent(".crow/recovery")
        let recovered = try FileManager.default.contentsOfDirectory(at: trash, includingPropertiesForKeys: nil)
        ...
        XCTAssertEqual(try String(contentsOf: migrated.appendingPathComponent("old.txt"), encoding: .utf8), "keep old recovery")
        model.navigateUp()
