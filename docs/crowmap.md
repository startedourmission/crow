...
**Add milestone** on a segment inserts a new milestone on that edge at the date you clicked (clamped to the segment). From a last milestone it still appends a week later. Every milestone with an incoming or outgoing timeline edge is main, including multiple milestones on the same date. Every milestone with an incoming or outgoing timeline edge is main, including multiple milestones on the same date. Colliding milestones stack vertically with their own labels and curved edges. Drag a milestone’s small connection point to another milestone to connect them; drag the node body horizontally to change its date, or vertically to change its priority — one gesture never changes both. Work notes cannot change date by dragging; they slide freely on the map like a graph view and settle without rewriting the note. Dragging a node to the edge of the map view scrolls in that direction. ⌘Z undoes the last map change (move, add, delete, connect, disconnect, color); ⌘⇧Z redoes it. Dragging or editing a milestone date past a connected neighbor gathers every crossed milestone on that path onto the same date; uncrossed milestones keep their dates. Hold **Shift** during a date drag to keep spacing instead. Work on those collapsed segments stays on that date. Work on a segment does not freeze the milestone; it is clamped onto the new segment after the move. Right-click the last milestone on a branch and choose **Add next milestone** to append a new milestone a week later. Right-click a milestone and choose **Delete milestone** to remove it and connect each previous milestone to each next one; work on the old segments stays on the new connection. Use **Disconnect milestones** in a segment popup to remove that edge from both notes' `previous`/`next` properties. Only a milestone with no remaining edges is faded; its Markdown file stays in the start note's `milestones` list. Work on a removed edge becomes a memo on its source milestone. Legacy `inactive_next`, `replaces`, and cached superseded states no longer deactivate connected milestones.

Date columns have a fixed width. Map controls choose the scale: Day, Week, Month, or Year. Date scale, line thickness, priority spacing, disconnected-milestone visibility, and work-note name visibility are remembered per map when its tab is closed and reopened. Day scale keeps a column for every day, week scale for every week start, and so on. When zoomed out, date labels thin to a regular interval so they do not overlap; the columns themselves stay. Day and week labels use MM/DD; month labels use the month name; year labels use the year. A single year label stays at the top left for day, week, and month scales and follows the visible range when scrolling horizontally. Today’s date column is tinted. Extra space above priority 1 keeps titles clear of the date axis. When a timeline’s start node is off the left of the view, its project name stays pinned at the left edge on that timeline. Dashed horizontal lines mark each priority lane the same way date columns are marked. During a date drag the target date column is highlighted; during a priority drag the milestone priority lane is highlighted. The start note connects to its first milestone with a straight, faint dashed line even when their priorities differ. A priority change stays level until that date, then takes a short S-curve inside the day’s column. Timelines that share a priority, including a branch off the same milestone, each have their own lane for the whole segment: the line moves onto that lane at the fork instead of sharing one stroke until the destination. Hovering a work note hides other work-note titles so they do not overlap. **Note names** in the controls popup shows or hides those titles; hidden names still appear on hover or selection. Work notes on the same date sit in a ring around that date on the timeline; **Note spacing** sets the ring radius (0 pins them to the edge). Notes on neighboring dates push apart when they overlap. Reduced-motion settings disable the animation. Node sizes reflect their connections, and the controls popup also adjusts line thickness and the vertical spacing between priority lanes. At minimum spacing the lanes sit nearly against each other.

Drag an empty region to select work notes, or use Command-click to toggle individual notes. Start notes, milestones and timeline edges are excluded from box selection. Right-click the selected group for bulk deletion or **Run Codex / Claude / Grok with N notes**. Local agent sessions live at `<map folder>/.sessions/<id>/`; `notes/` contains symlinks to the chosen original Markdown files. Agents receive structural guidance from the map’s `AGENTS.md` and a session-specific selection list. Agent tabs carry explicit Crowmap ownership; conversations stay in the right history panel, while the left Crowmap list, shown only while Crowmap is enabled in Settings, shows each map’s upcoming milestones from today onward, nearest first, with the project name under the title; clicking a row focuses that node on the map. Inside the map, a project-name chip stays on the left of any timeline whose start node has scrolled off-screen, at that timeline’s current Y. The agent history panel aggregates the map’s session directories; opening a saved conversation resumes its original selection directory. New unsaved sessions appear immediately, and older map agents are recognized by their session path. Existing user-written instructions are preserved. Remote device notes remain read-only and cannot be selected for a local agent session.
...
The adjacent controls button toggles a popup containing search, disconnected milestone visibility, zoom, fit, refresh, line thickness, date scale, and priority spacing. Zoom buttons keep the current viewport center anchored; Cmd/Ctrl-wheel zoom keeps the point under the pointer anchored, including over nodes and date labels. Fit resets the view to the timeline origin. Work-note context menus create linked notes or delete the selected note; deleted Markdown is retained outside the flat note root in `~/.crow/crowmap-deleted`.

## Copying and ordering

Right-click a start note, milestone, or timeline edge to change that timeline’s color. The color lives in the `.crowmap` display cache and does not edit Markdown. Every Markdown node has **Copy** and **Duplicate** in its context menu. Copy places actual Markdown file URLs on the system clipboard. Duplicate immediately creates numbered files in the current map. A work note or milestone copies only its own file; a start note includes all its project's milestone files, including disconnected milestones, but never attached work notes or device nodes. Timeline duplicates remap internal links to the new filenames. Existing body links remain references, not recursive copies. Remote work notes duplicate as local snapshots.

Priority 1 is the top row; a lower number sits above a higher number among projects active on that date. Drag one milestone onto another on the same date to swap their timeline order, including `previous`/`next` links. Dates and priorities stay the same. Dragging to a different project rank sets one priority for all milestones at that project/date, avoiding contradictory simultaneous priorities. Hold **Command** during that priority drag to apply the new priority to all later milestones in the same project as well. Earlier milestones and other projects keep their stored properties.
