...
[프로젝트 소개](../README.md) · [개발·테스트](DEVELOPMENT.md)
...
iPhone·iPad는 iOS/iPadOS 18 이상을 지원하며, 소스 빌드는 [개발 안내](DEVELOPMENT.md)를 참고하세요.
...
연결 종료나 앱 재실행 후에는 끊긴 탭의 **Reconnect** 버튼이나 탭 메뉴의 **Restart Reverse Agent…**로 통로와 그 탭의 대화를 다시 엽니다.
서버에서 그 탭의 대화를 특정할 수 있으면 이어서 재개하고, 같은 폴더에서 어느 탭의 대화인지 구분할 수 없으면 새 대화를 시작합니다.
다른 저장 대화는 에이전트 패널에서 재개·분기할 수 있습니다.
...
- SSH 터미널에서 **Ctrl+V**를 누르면 클립보드 이미지를 서버의 임시 PNG 파일로
  ...
  역방향 SSH 에이전트도 서버 임시 PNG로 올려서, 서버에서 돌아가는 CLI가 다른 SSH 세션과 같이 이미지로 붙입니다.
...
- iPhone 하단의 **Snippets** 버튼에서 텍스트를 저장하고 목록을 탭하면 현재 커서에
  ...
  저장한 텍스트 뒤에 Enter를 추가하지 않습니다. 목록의 복사 버튼은 스니펫 텍스트를 클립보드에 넣습니다.
  목록 관리는 설정에서도 가능합니다.
...
- iPad 워크스페이스 선택기는 Mac처럼 왼쪽 패널 하단에 있습니다. 오른쪽 패널의
  ...
  **Projects**로 목록에 돌아가 다른 저장소를 선택할 수 있습니다.
  ...
  계정 ID와 토큰을 기기 Keychain에 저장·수정·삭제할 수 있습니다. 설정 화면을 여는 것만으로는 Keychain을 읽지 않습니다.
  저장한 계정 이름은 표시할 수 있고, 토큰은 불러오기·교체·삭제하거나 기능이 사용할 때만 읽습니다. 저장할 때 네트워크 인증은 하지 않습니다.
